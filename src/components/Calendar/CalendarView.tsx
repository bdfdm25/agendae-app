...
export default function CalendarView({ selectedDay, days, currentMonth, onDateSelect, onMonthChange }) {
    const [date, setDate] = useState(new Date().getDate());
    const [monthDays, setMonthDays] = useState([]);
    const [month, setMonth] = useState(LocaleConfig.monthNames[new Date().getMonth()]);
    ...
    const getItemLayout = (data, index) => ({ length: 45, offset: 55 * index, index });
    ...
    return (
        <View style={styles.rootContainer}>
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                <IconButton onPress={() => onMonthChange("previous", month)} icon="arrow-back-circle-sharp" size={24} />
                <Text style={GlobalStyles.sectionTitle}>{month}</Text>
                <IconButton onPress={() => onMonthChange("next", month)} icon="arrow-forward-circle-sharp" size={24} />
            </View>
