...
export default function Calendar() {
    ...
    const monthDays = getMonthDays(today.getFullYear(), today.getMonth(), today.getDate());
    const [selectedDay, setSelectedDay] = useState(today.getDate());
    const [days, setDays] = useState(monthDays);
    const [currentMonth, setCurrentMonth] = useState(LocaleConfig.monthNames[today.getMonth()]);
    ...
    function getSelectedMonth(direction: string, selectedMonth: string) {
        const monthNumber = LocaleConfig.monthNames.findIndex((month) => month === selectedMonth);
        switch (direction) {
            case "previous":
                if (selectedMonth === "Janeiro") {
                    setDays(getMonthDays(currentYear - 1, 11));
                    setCurrentMonth(LocaleConfig.monthNames[11]);
                    setCurrentYear(currentYear - 1);
                } else {
                    setDays(getMonthDays(currentYear, monthNumber - 1));
                    setCurrentMonth(LocaleConfig.monthNames[monthNumber - 1]);
                }
                break;
            case "next":
                if (selectedMonth === "Dezembro") {
                    setDays(getMonthDays(currentYear + 1, 0));
                    setCurrentMonth(LocaleConfig.monthNames[0]);
                    setCurrentYear(currentYear + 1);
                } else {
                    setDays(getMonthDays(currentYear, monthNumber + 1));
                    setCurrentMonth(LocaleConfig.monthNames[monthNumber + 1]);
    ...
    return (
        <CalendarView
            ...
            onMonthChange={(direction: string, selectedMonth: string) => getSelectedMonth(direction, selectedMonth)}
        />
    );
}
